...
from dpo_trl_length import DPOTrainer
...
class ScriptArguments:
    ...
    positive_k: Optional[int] = field(default=1, metadata={"help": "the number of positive responses sampled for lambdarank list loss"})
    negative_k: Optional[int] = field(default=1, metadata={"help": "the number of negative responses sampled for lambdarank list loss"})
...
def prepare_data(
    ...
    for sample in ds:
        ...
        elif choose_type == "max_min_list":
            sorted_indices = np.argsort(sample["rewards"])
            idx0 = sorted_indices[-positive_k:]
            idx1 = sorted_indices[:negative_k]
        ...
        if type(idx0) == np.ndarray or type(idx0) == list:
            ...
            #         prompts.append(P)
            #         pos.append(sample["responses"][idx0[i]] + eot_token)
            #         neg.append(sample["responses"][idx1[j]] + eot_token)
            #         margin.append((sample["rewards"][idx0[i]] - sample["rewards"][idx1[j]]) * margin_scale)
        else:
...
if __name__ == "__main__":
    ...
    train_dataset = prepare_data(
        ...
        positive_k=script_args.positive_k,
        negative_k=script_args.negative_k,
    )
