...
class DPOTrainer(Trainer):
    ...
    def dpo_loss(
        ...
        size_completion: torch.LongTensor,
        ...
        if self.f_divergence_type == FDivergenceType.ALPHA_DIVERGENCE.value:
            ...
            chosen_logratios = (policy_chosen_logps - reference_chosen_logps) / size_completion.float()[0]
            reject_logratios = (policy_rejected_logps - reference_rejected_logps) / size_completion.float()[1]

            pi_logratios = chosen_logratios.to(self.accelerator.device) 
            ref_logratios = reject_logratios.to(self.accelerator.device) 
    ...
    def concatenated_forward(
        ...
        if self.aux_loss_enabled:
            return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, nll_loss, outputs.aux_loss, size_completion)

        return (chosen_logps, rejected_logps, chosen_logits, rejected_logits, nll_loss, size_completion)
    ...
    def get_batch_loss_metrics(
        ...
        size_completion = forward_output[-1]
        if self.aux_loss_enabled:
