'''
conda create --name pairrm python==3.10
...
#     truncation=True,
# )
rm_pipe = llm_blender.Blender()
rm_pipe.loadranker("llm-blender/PairRM")
...
def get_reward(prompts, responses):
    rewards = rm_pipe.rank(prompts, responses, return_scores=True, batch_size=1)
    # rewards = pipe_outputs[0]
    return rewards
...
with torch.no_grad():
    ...
    input_prompts = [sample['prompt'][0]['content'] for sample in ds]
    responses = ds["responses"]
        
    rewards = get_reward(input_prompts, responses)
        
    for i, sample in enumerate(ds):
        data.append({"prompt": sample["prompt"], "responses": sample["responses"], "rewards": rewards[i].tolist()})
    ...
    #     input_prompt = sample['prompt'][0]['content']
        
    #     rewards = get_reward([input_prompt], [sample['responses']]).tolist()        
